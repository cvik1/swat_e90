...
# env = gym.envs.make("RoboschoolAnt")
...
with tf.Session() as sess:
    ...
    for episode in range(num_episodes):
        ...
        if (episode+1)%100==0:
            monitor = Monitor(env, "MountainCar/"+str(episode), force=True)
            state = monitor.reset()
            done = False
            sum_reward = 0
            while not done:
                ...
                # next_state shape=(2,)
                #env.step() requires input shape = (1,)
                next_state, reward, done, _ = monitor.step(
                                        np.squeeze(action, axis=0))

                state = next_state

                sum_reward += reward
            print("rendering episode reward: " + str(sum_reward))
